...
namespace samplesutil {
    ...
    template <typename F>
    __device__ F* GetSubMatrix(F* A, int row, int col,
                     unsigned int BLOCK_SIZE, unsigned int DIM) {
    ...
    static __host__ void prepareRandomNumbersCPUGPU(
        unsigned int N, std::vector<float>& vals, float** dValsPtr
        ) {
