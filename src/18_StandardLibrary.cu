...
__managed__ cuda::std::atomic<int> mResultCPU{ 0 }, mResultGPU{ 0 };
...
__global__ void completeReductionGPU(int N, const int* input)
{
	// Launchpad code for calling the architecutre-agnostic reduction function
	int tId = blockIdx.x * blockDim.x + threadIdx.x;
	reduceAtomic(tId, N, N, input, mResultGPU);
}

template<unsigned int NUM_THREADS>
__host__ void completeReductionCPU(int N, int* mNumbers)
{
	/*
	...
	*/
	...
	for (int i = 0; i < threads.size(); i++)
		threads[i] = std::thread(reduceAtomic, i, NUM_THREADS, N, mNumbers, std::ref(mResultCPU));
...
int main()
{
	...
	*/
	...
	// Output both results
	std::cout << "Reduction result CPU: " << mResultCPU << "\n" << std::endl;
	std::cout << "Reduction result GPU: " << mResultGPU << "\n" << std::endl;
...
the default cuda::std::atomic and the cuda::atomic with block thread_scope.
*/
